...
function getMenuPermission($name, $com, $act, $type)
{
    $add_class = '';
    if (phanquyen_tv($com, $_SESSION['login']['quyen'], $act, $type) == 1) {
        if ($com == $_GET['com'] && $act == $_GET['act'] && $type == $_GET['type']) {
            $add_class = 'class="this"';
        }
        echo  "<li " . $add_class . "><a href='index.php?com=" . $com . "&act=" . $act . "&type=" . $type . "'>" . $name . "</a></li>";
    }
...
{
    return md5(sha1($token . $str . $salt));
}
...
function redirect_router($url, $statusCode = 301)
{
